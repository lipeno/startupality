class ScriptFile < ActiveRecord::Base
  		
  	belongs_to :script
 	attr_accessible  :script_file,:script_id,:script_file_file_name
 	...
 	has_attached_file :script_file
 	#validates_attachment_content_type :script_file, :content_type =>['application/pdf']
 	def get_file_url
		self.script_file.url
	end
	def file_extension
		File.extname(self.script_file_file_name).delete "."
	end
